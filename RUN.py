...
# global CourseUrl
# global FinishUrl
# CourseUrl = "https://changjiang.yuketang.cn/v2/api/web/courses/list?identity=2"
# FinishUrl = "https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/course/pub_new_pro"
...
class run:
    def __init__(self, cookies: requests.cookies.RequestsCookieJar):
        ...
        self.URL_getCourse = "https://changjiang.yuketang.cn/v2/api/web/courses/list?identity=2"
        self.URL_getCuts = "https://changjiang.yuketang.cn/v2/api/web/logs/learn/" + \
            self.REPLACE+"?actype=-1&page=0&offset=20"
        self.URL_getVideo = "https://changjiang.yuketang.cn/c27/online_courseware/xty/kls/pub_news/"+self.REPLACE+"/"
        self.URL_getDone = "https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/course/pub_new_pro"
        self.URL_getVideoInfo = "https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/leaf_info/" + \
            self.REPLACE_CLASSROOMID+"/"+self.REPLACE+"/"
        self.URL_getVideoUrl = "https://changjiang.yuketang.cn/api/open/audiovideo/playurl"
        self.URL_heartBeat = "https://changjiang.yuketang.cn/video-log/heartbeat/"
    ...
    def getCuts(self):
        ...
        '''
        # url = f"https://changjiang.yuketang.cn/v2/api/web/logs/learn/{self.classroom_id}?actype=-1&page=0&offset=20"
    ...
    def getVideo(self):
        ...
        '''
        for i in self.cuts:
            # url = f"https://changjiang.yuketang.cn/c27/online_courseware/xty/kls/pub_news/{i}/"
            url = self.URL_getVideo.replace(self.REPLACE, str(i))
            res = requests.get(
                url, headers=self.FinishHeaders, cookies=self.cookies)
            js = res.json()
            if js["success"]:
                tmp = self._getVideo(res.json())
                self.videos += tmp[0]
                self.reads += tmp[1]
            else:
                print(js)
    ...
    def getDone(self):
        ...
        获取所有已完成内容
        '''
        # url = "https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/course/pub_new_pro"
        url = self.URL_getDone
        data = {
            "cid": str(self.classroom_id),
            "new_id": self.cuts
        }
        res = requests.post(url, headers=self.FinishHeaders,
                            cookies=self.cookies, data=json.dumps(data))
        self.done = res.json()
    ...
    def getUndone(self):
        ...
        if self.done:
            totalDone = {}
            for i in self.done["data"]:
                cut = self.done["data"][i]
                for i in cut:
                    totalDone[i] = cut[i]
            ...
            for i in self.videos:
                try:
                    ...
                    self.undonevideos += [i]
            print(f"{len(self.undonevideos)} 个未完成")
            ...
            for i in self.reads:
                try:
                    ...
                    self.undonereads += [i]
            print(f"{len(self.undonereads)} 个未完成")
    ...
    def getVideoInfo(self, video):
        ...
        '''
        id_ = video["id"]
        # url = f"https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/leaf_info/11006920/{id_}/"
    ...
    def _readContent(self, leaf_data):
        ...
        url = f"https://changjiang.yuketang.cn/mooc-api/v1/lms/learn/user_article_finish/{content_id}/?cid={classroom_id}&sid={sku_id}"
        res = requests.get(url, headers=self.FinishHeaders,
                           cookies=self.cookies)
        return res
    ...
    def flushALL(self):
        ...
        for i in self.undonevideos:
            ...
            if not flushed:
                print(i["title"], Tools.color("未成功", "red"))
                continue
            print("\nWait for 3s")
            time.sleep(3)
...
if __name__ == "__main__":
    dic = {"csrftoken": "5VfQwY05UkZ4hwQdCPbP98ClvjbVcPtb", "django_language": "zh-cn",
           "login_type": "WX", "sessionid": "2pnlx9ymsxxg63alaoyqgq0pyot5rmzh"}
    mission = run(dic)
